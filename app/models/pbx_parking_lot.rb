class PbxParkingLot < RestModel
    ...
    property :timeout_extension

    validates :pbx_account_id, presence: true
    validates :extension, presence: true
    validates :timeout, presence: true
    validates_inclusion_of :timeout, :in => 0..3600
    validates :timeout_extension, presence: true
    validates :moh, presence: true
    ...
    def self.load_by_user(currentUser)
        ...
        PbxParkingLot.from_source(response.parsed_response)
    end

    def self.load_from_json(json)
        PbxParkingLot.from_source(JSON.parse(json)).first
    end
    ...
    def baseURL
        "/api/v1/portal_pbx_parking_lots"
    end
